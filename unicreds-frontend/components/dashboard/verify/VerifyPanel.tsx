...
export default function VerifyPanel({ dict, locale }: VerifyPanelProps) {
  ...
  const [result, setResult] = useState<OnChainCredential | null>(null);
  ...
  async function handleVerify() {
    ...
    try {
      const credential = await fetchCredentialByQuery(query);
      if (!credential) {
    ...
    <>
      ...
      {result && (
        <div className="animate-fade-in-up">
          ...
          <div className={`glass-card rounded-3xl p-8 flex flex-col md:flex-row gap-8 items-start ${result.isValid ? "ltr:border-l-4 ltr:border-l-green-500 rtl:border-r-4 rtl:border-r-green-500" : "ltr:border-l-4 ltr:border-l-red-500 rtl:border-r-4 rtl:border-r-red-500"}`}>
            <div className={`flex-shrink-0 flex flex-col items-center justify-center w-24 h-24 rounded-2xl ${result.isValid ? "bg-green-50 text-green-600" : "bg-red-50 text-red-600"}`}>
              <SealCheck weight="fill" className="w-9 h-9 mb-1" />
              <span className="text-xs font-bold uppercase tracking-wide">
                {result.isValid ? dict.valid : labels.revoked}
              </span>
            </div>
            ...
            <div className="flex-1 w-full grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-12">
              <div>
                ...
                <p className="font-serif text-xl text-brand-dark">{result.issuerName}</p>
              </div>
              <div>
                <p className="text-xs text-gray-400 uppercase tracking-wider mb-1">{labels.issuerAddress}</p>
                <p className="text-xs text-gray-500 font-mono mt-1 break-all">{result.issuerAddress}</p>
              </div>
              <div>
                <p className="text-xs text-gray-400 uppercase tracking-wider mb-1">{dict.fields.studentAddress}</p>
                <p className="text-sm font-mono text-gray-700 bg-gray-100 p-2 rounded-lg break-all">{result.studentAddress}</p>
              </div>
              <div>
                <p className="text-xs text-gray-400 uppercase tracking-wider mb-1">{dict.fields.hash}</p>
                <p className="text-sm font-mono text-gray-700 bg-gray-100 p-2 rounded-lg break-all">{result.hash}</p>
              </div>
              <div>
                <p className="text-xs text-gray-400 uppercase tracking-wider mb-1">{labels.cid}</p>
                <p className="text-sm font-mono text-gray-700 bg-gray-100 p-2 rounded-lg break-all">{result.ipfsCID}</p>
              </div>
              <div>
                <p className="text-xs text-gray-400 uppercase tracking-wider mb-1">{dict.fields.issueDate}</p>
                <p className="text-sm text-gray-700">{formatTimestamp(result.issueTimestamp, locale)}</p>
              </div>
              {!result.isValid && result.revocationReason && (
                <div className="md:col-span-2">
                  <p className="text-xs text-gray-400 uppercase tracking-wider mb-1">{labels.reason}</p>
                  <p className="text-sm text-red-600 bg-red-50 border border-red-100 p-3 rounded-lg">{result.revocationReason}</p>
                </div>
              )}
              <div>
                <a
                  href={getIpfsUrl(result.ipfsCID)}
                  target="_blank"
                  rel="noreferrer"
